...
__global__ void rasterizing(int n, int w, int h,
    ...
    if (index < n) {
		...
		glm::vec3 vertices[3] = {tri.v[0].pos, tri.v[1].pos, tri.v[2].pos};
		AABB aabb = getAABBForTriangle(vertices);
		...
		for (int i = glm::max(0.0f, pixel_min.x); i <= pixel_max.x; i++) {
			for (int j = glm::max(0.0f, pixel_min.y); j <= pixel_max.y; j++) {
				...
				glm::vec3 barycentric = calculateBarycentricCoordinate(vertices,
					glm::vec2(x,y));
...
__global__ void fragmentsToDepth() {
}

__global__ void depthTesting() {
}

__global__ void fragmentToBuffer() {
}
