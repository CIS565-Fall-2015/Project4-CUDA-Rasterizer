...
static int width = 0;
static int height = 0;
static int *dev_bufIdx = NULL; // indices buffer
static VertexIn *dev_bufVertex = NULL; // buffer of untransformed vertices
static VertexOut *dev_shadedVertices = NULL; // buffer of transformed vertices. need?
static Triangle *dev_primitives = NULL; // buffer of primitives
static FragmentIn *dev_fragsIn = NULL; // buffer of unshaded fragments.
static FragmentOut *dev_fragsOut = NULL; // buffer of shaded fragments. basically, pixels. need?
static glm::vec3 *dev_framebuffer = NULL; // color frame buffer. need?
 ...
 */
void minRasterizeInit(int w, int h) {
    width = w;
    height = h;
	...
	if (i < numPrimitives) {
		...
		for (int y = BBYmin; y < BBYmax; y++) {
			for (int x = BBXmin; x < BBXmax; x++) {
				...
				if (x == -10 && y == -10 && dev_primitives[i].v[0].nor.y < 0.0f) { // debug
					printf("zDepth of bottom face is %f\n", zDepth);
				}
				...
				float peekDepth = dev_frags[fragIndex].depth; // debug
				bool closer = zDepth < peekDepth; // debug
...
__global__ void minFragmentShading(int numFrags, FragmentIn *dev_fragsIn, FragmentOut *dev_fragsOut) {
	int fragIndex = 311589; // debug
	float peekDepth = dev_fragsIn[fragIndex].depth; // debug
