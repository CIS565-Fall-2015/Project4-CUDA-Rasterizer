...
namespace Efficient {
	...
	__global__ void filter(int *odata, const Triangle *idata, const int n){
		int k = blockIdx.x*blockDim.x + threadIdx.x;
		if (k < n){
			odata[k] = (int)idata[k].isValidGeom;
		}
	}

	__global__ void scatter(Triangle *odata, const Triangle *idata, const int *filter, const int *idx, const int n){
		int k = blockIdx.x*blockDim.x + threadIdx.x;
...
void scan(int n, int *odata, const int *idata) {
	...
	if (n <= BLOCKSIZE){
		...
		checkCUDAError1("SC small scan base case1");
		// Scan on each block & get total sum
		smallScan << <1, BLOCKSIZE >> >(scan_out, idata, n);
		checkCUDAError1("SC small scan base case2");
		cudaMemcpy(odata, scan_out, n * sizeof(int), cudaMemcpyDeviceToDevice);

		checkCUDAError1("SC small scan base case3");
	}
	else {
		...
		checkCUDAError1("SC get block total");
 ...
 */
void compact(int n, int* f, int* idx, Triangle *dv_out, Triangle *idata, int* c) {
	// Padding
	//int gridSize = (n + BLOCKSIZE - 1) / BLOCKSIZE;
	...
	if (tsize > 1){
		...
		scatter << <tsize, BLOCKSIZE*2 >> >(dv_out, idata, f, idx, n);
	}
